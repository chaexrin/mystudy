...
public class MemberMenu {
  ...
  static void add() {
    ...
    member.registeredDate = Prompt.input("가입일? ");
  ...
  static void modify() {
    ...
    member.registeredDate = Prompt.input("가입일(%s)? ", member.registeredDate);
  }
  ...
  static void list() {
    ...
    System.out.printf("%-20s\t%10s\t%s\n", "email", "name", "registeredDate");

    for (int i = 0; i < length; i++) {
      Member member = members[i];
      System.out.printf("%-20s\t%10s\t%s\n", member.email, member.name, member.registeredDate);
    }
  }
}
