...
public class AssignmentModifyHandler implements MenuHandler {
  ...
  public void action(Menu menu) {
    ...
    assignment.title = prompt.input("과제명(%s)? ", oldassignment.title);
    assignment.content = prompt.input("내용(%s)? ", oldassignment.content);
    assignment.deadline = prompt.input("제출 마감일(%s)? ", oldassignment.deadline);
