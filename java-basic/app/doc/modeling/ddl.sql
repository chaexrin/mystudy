...
CREATE TABLE lms_application (
  ...
  lecture_no  INTEGER  NOT NULL COMMENT '과정 번호', -- 과정 번호
...
ALTER TABLE lms_application
  MODIFY COLUMN apply_no INTEGER NOT NULL AUTO_INCREMENT COMMENT '수강신청번호';

-- 과정
CREATE TABLE lms_lectures (
  lecture_no INTEGER      NOT NULL COMMENT '과정 번호', -- 과정 번호
  title      VARCHAR(255) NOT NULL COMMENT '과정 명', -- 과정 명
  ...
  support    BOOLEAN      NOT NULL COMMENT '국비 지원 여부', -- 국비 지원 여부
  room_no    INTEGER      NULL     COMMENT '강의실 번호', -- 강의실 번호
...
ALTER TABLE lms_lectures
  MODIFY COLUMN lecture_no INTEGER NOT NULL AUTO_INCREMENT COMMENT '과정 번호';
...
CREATE TABLE lms_rooms (
  room_no   INTEGER     NOT NULL COMMENT '강의실 번호', -- 강의실 번호
  center_no INTEGER     NOT NULL COMMENT '교육센터번호', -- 교육센터번호
  title     VARCHAR(60) NOT NULL COMMENT '강의실 명', -- 강의실 명
  qty       INTEGER     NOT NULL COMMENT '수용 가능 인원' -- 수용 가능 인원
)
COMMENT '강의실';
...
ALTER TABLE lms_rooms
  MODIFY COLUMN room_no INTEGER NOT NULL AUTO_INCREMENT COMMENT '강의실 번호';

-- 강사
CREATE TABLE lms_teachers (
  member_no INTEGER      NOT NULL COMMENT '강사번호', -- 강사번호
...
CREATE TABLE lms_photos (
  photo_no INTEGER      NOT NULL COMMENT '강의실 사진번호', -- 강의실 사진번호
  filepath VARCHAR(255) NOT NULL COMMENT '사진 경로', -- 사진 경로
  room_no  INTEGER      NOT NULL COMMENT '강의실 번호' -- 강의실 번호
)
COMMENT '강의실 사진';
...
ALTER TABLE lms_photos
  ADD CONSTRAINT PK_lms_photos -- 강의실 사진 기본키
  PRIMARY KEY (
  photo_no -- 강의실 사진번호
  );

ALTER TABLE lms_photos
  MODIFY COLUMN photo_no INTEGER NOT NULL AUTO_INCREMENT COMMENT '강의실 사진번호';

-- 교육센터 
CREATE TABLE lms_centers (
  center_no INTEGER     NOT NULL COMMENT '교육센터번호', -- 교육센터번호
  title     VARCHAR(60) NOT NULL COMMENT '지점 명' -- 지점 명
)
COMMENT '교육센터 ';
...
CREATE TABLE lms_students (
  ...
  level_no       INTEGER      NULL     COMMENT '학력 번호', -- 학력 번호
...
CREATE TABLE lms_level (
  level_no INTEGER     NOT NULL COMMENT '학력 번호', -- 학력 번호
...
CREATE TABLE lms_departments (
  department_no INTEGER     NOT NULL COMMENT '부서번호', -- 부서번호
  title         VARCHAR(60) NOT NULL COMMENT '부서' -- 부서
)
COMMENT '부서';
...
-- 강의실 사진
ALTER TABLE lms_photos
  ADD CONSTRAINT FK_lms_rooms_TO_lms_photos -- 강의실 -> 강의실 사진
  FOREIGN KEY (
  room_no -- 강의실 번호
  )
  REFERENCES lms_rooms ( -- 강의실
